...
const Step = ({ children, errors, icon, onNext, onPrevious, show, step, title, ...rest }) => {
    ...
    return React.Children.toArray(children).some(child => {
      if (child.type.name === "FormGroup") {
        innerFound = React.Children.toArray(child.props.children).some(
          inner => errors[inner.props.name] || (inner.props.required && !inner.props.value)
        );
      }
      return innerFound || errors[child.props.name] || (child.props.required && !child.props.value);
    });
  }, [children, errors]);
